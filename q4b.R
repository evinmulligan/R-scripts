...
AIClist <- lapply(fits, AIC, k = 2)   #  calculate AIC for each model
AICs <- unlist(AIClist)     

Results <- cbind(df,AICs)     #   save models,AICs in dataframe for display
SortedResults <- Results[order(Results$AICs, decreasing = FALSE),]
print(SortedResults)
